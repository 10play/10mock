...
import { Method, AddRouteBody, Route } from "./types";
...
const extractRouteKey = (route: string): { path: string; method: Method } => {
...
const removeMiddlewares = (
  ...
  if (route.route)
    route.route.stack.forEach((route: any, i: number, routes: any) =>
      removeMiddlewares(route, i, routes, method, path)
    );
};
...
app.get(adminPath, (req, res) => {
  ...
  if (queryMethod && queryPath) {
    ...
    const { count, method, path, stubRequests, response } =
      mockHandlers[routeKey];
    return res.json({
      count,
      method,
      path,
...
app.post(adminPath, (req, res) => {
  const { method, path, response }: AddRouteBody = req.body;
  console.log(`New mock for route: [${method}]${path}`);
  if (!method || !path || !response) {
    return res
      .status(500)
      .json("Should provide : !method || ! path || !response");
  }
  ...
  mockHandlers[newRouteKey] = {
    ...
    middleware: (req: any, res: any) => {
      ...
      mockHandlers[newRouteKey].stubRequests.push({
        params: req.params,
        query: req.query,
        body: req.body,
        headers: req.headers,
      ...
      console.log(
        `Mock [${method}]${path} response with body:`,
...
app.delete(adminPathDeleteAll, (req, res) => {
  try {
    for (const handler in mockHandlers) {
      ...
      const routes = app._router.stack;
      routes.forEach((route: Route, i: number, routes: Route[]) =>
        removeMiddlewares(route, i, routes, method, path)
      );
    }
